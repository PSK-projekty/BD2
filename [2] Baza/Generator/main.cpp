...
double randomDouble (double min_num, double max_num) {
	double result = 0.0;
	double low_num = min_num;
	double hi_num = max_num;
	result = ( (double) rand () / RAND_MAX ) * ( hi_num - low_num ) + low_num;
	return result;
}
...
void samochod () {
	...
	for(i = 1; i <= 1000; i++ ) {
		...
		int ID_Wynajem_Samochod = random (0, 10000);
		int ID_Dodatkowe_Samochod = random (1, 10000);
		int ID_Czesci_Samochod = random (1, 100000);
		int ID_Wyposazenie = random (1, 500);
		int ID_Akcesoria_Samochod = random (1, 10000);
		...
		int Przebieg = random (0, 20000);
		double Cena = random (5000, 200000);

		fprintf (plik, "%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%s,%s,%s,%s,%d,%.2lf\n", ID_Samochodu, ID_Model,ID_Silnik, ID_Salonu, ID_Wersja, ID_Wynajem_Samochod, ID_Dodatkowe_Samochod, ID_Czesci_Samochod, ID_Wyposazenie, ID_Akcesoria_Samochod, Naped, Kolor, VIN, RokProdukcji, Przebieg, Cena);
	}
...
void pracownicy () {
	...
	for( i = 1; i <= 500; i++ ) {
		...
		int ID_Wynajem_Pracownicy = random (1, 10000);
		char Imie[100];
		...
		double Pensja = randomDouble (3000.0, 25000.0);

		fprintf (plik, "%d,%d,%d,%d,%d,%s,%s,%s,%d,%.2lf\n", ID_Pracownik, ID_Salon, ID_Adres, ID_Stanowisko, ID_Wynajem_Pracownicy, Imie, Nazwisko, Email, Telefon, Pensja);
	}
...
void klient () {
	...
	for(i = 1; i <= 1000; i++ ) {
		int ID_Klient = i;
		int ID_Adres = random (1, 200);
		int ID_Wynajem_Klient = random (1, 10000);
		char Imie[100];
		sprintf (Imie, "Imie %d", i);
		char Nazwisko[100];
		sprintf (Nazwisko, "Nazwisko %d", i);
		...
		fprintf (plik, "%d,%d,%d,%s,%s,%s,%d\n", ID_Klient, ID_Adres, ID_Wynajem_Klient, Imie, Nazwisko, Email, Telefon);
	}
...
void jazda_probna () {
	...
	for( int i = 1; i <= 1000; i++ ) {
		...
		fprintf (plik, "%d,%d,%d,%d,%04d-%02d-%02d\n", ID_JazdaProbna, ID_Samochod, ID_Pracownik, ID_Klient,
...
void sprzedaz () {
	...
	for(i = 1; i <= 10000; i++ ) {
		...
		int ID_Klient = random (1, 1000);
		int ID_Wyposazenie = ( random (1, 10) <= 7 ) ? random (1, 500) : -1; // 70% szansa na NULL
		...
		if( ID_Samochod != -1 )
			...
			strcpy (ID_Samochod_str, "NULL");

		if( ID_Akcesorium != -1 )
			snprintf (ID_Akcesorium_str, sizeof (ID_Akcesorium_str), "%d", ID_Akcesorium);
		else
			strcpy (ID_Akcesorium_str, "NULL");

		if( ID_Wyposazenie != -1 )
			snprintf (ID_Wyposazenie_str, sizeof (ID_Wyposazenie_str), "%d", ID_Wyposazenie);
		else 
			strcpy (ID_Wyposazenie_str, "NULL");
		
		if( ID_Czesc != -1 ) 
			snprintf (ID_Czesc_str, sizeof (ID_Czesc_str), "%d", ID_Czesc);
		else 
			strcpy (ID_Czesc_str, "NULL");

		if( ID_Wynajem != -1 ) 
			snprintf (ID_Wynajem_str, sizeof (ID_Wynajem_str), "%d", ID_Wynajem);
		else 
			strcpy (ID_Wynajem_str, "NULL");
		
		if( ID_Wersja != -1 ) 
			snprintf (ID_Wersja_str, sizeof (ID_Wersja_str), "%d", ID_Wersja);	
		else 
			strcpy (ID_Wersja_str, "NULL");

		if( ID_Naprawa != -1 )
			snprintf (ID_Naprawa_str, sizeof (ID_Naprawa_str), "%d", ID_Naprawa);
		else
			strcpy (ID_Naprawa_str, "NULL");


		fprintf (plik, "%d,%s,%s,%d,%d,%s,%s,%s,%s,%s,%04d-%02d-%02d,%.2f\n", ID_Sprzedaz, ID_Samochod_str, ID_Akcesorium_str,ID_Pracownik, ID_Klient, ID_Wyposazenie_str, ID_Czesc_str, ID_Wynajem_str, ID_Wersja_str, ID_Naprawa_str, Data.tm_year + 1900, Data.tm_mon + 1, Data.tm_mday, Kwota);
	}
...
void akcesoria () {
	...
	for (int i = 1; i <= 300; i++) {
        int ID_Akcesorium = i;
        int ID_Akcesoria_Samochod = random(1, 1000);
        char Nazwa[100];
        sprintf(Nazwa, "Akcesorium %d", i);
        float Cena = randomDouble(50.00, 1500.00);

        fprintf(plik, "%d,%d,%s,%.2f\n", ID_Akcesorium, ID_Akcesoria_Samochod, Nazwa, Cena);
    }
...
void wersja_wyposazenia () {
	...
	for(i = 1; i <= 100; i++ ) {
		...
		float Cena = randomDouble (5000.0, 40000.0);
...
void wynajem () {
	...
	for( i = 1; i <= 1000; i++ ) {
		...
		int ID_Wynajem_Klient = random (1, 10000);
		int ID_Wynajem_Pracownicy = random (1, 10000);
		int ID_Wynajem_Samochod = random (1, 100000);
		...
		// 70% szans
		if( range <= 70 )
			Cena = randomDouble (100.00, 800.00);

		// 30% szans
		else
			Cena = randomDouble (800.00, 2000.00);
		...
		if( DataZwrotu.tm_year == -1 ) {
			snprintf (DataZwrotuStr, sizeof (DataZwrotuStr), "NULL");
		}
		else {
			snprintf (DataZwrotuStr, sizeof (DataZwrotuStr), "%04d-%02d-%02d", DataZwrotu.tm_year + 1900, DataZwrotu.tm_mon + 1, DataZwrotu.tm_mday);
		}

		fprintf (plik, "%d,%d,%d,%d,%04d-%02d-%02d,%s,%.2f\n", ID_Wynajem, ID_Wynajem_Klient, ID_Wynajem_Pracownicy, ID_Wynajem_Samochod, DataWynajmu.tm_year + 1900, DataWynajmu.tm_mon + 1, DataWynajmu.tm_mday, DataZwrotuStr, Cena);
	}
...
void czesci () {
	...
	for(i = 1; i <= 500; i++ ) {
		int ID_Czesc = i;
		int ID_Czesci_Samochod = random (1, 10000);
		char Nazwa[101];
		snprintf (Nazwa, sizeof (Nazwa), "Czêœæ %d", i);
		char Numer[16];
		generateNumer (Numer);
		double Cena = randomDouble (100.0, 3000.0);

		fprintf (plik, "%d,%d,%s,%s,%.2f\n", ID_Czesc, ID_Czesci_Samochod, Nazwa, Numer, Cena);
	}
...
void dodatkowe_wyposazenie () {
	...
	for(i = 1; i <= 500; i++ ) {
		...
		int ID_Dodatkowe_Samochod = random (1, 10000);
		char Nazwa[101];
		snprintf (Nazwa, sizeof (Nazwa), "Wyposa¿enie %d", i);
		double Cena = randomDouble (50.0, 1500.0);

		fprintf (plik, "%d,%d,%s,%.2f\n", ID_Wyposazenie, ID_Dodatkowe_Samochod, Nazwa, Cena);
	}
...
void magazyn () {
	...
	for(i = 1; i <= 1000; i++ ) {
		...
		int ID_Akcesorium = random (1, 500);
...
void wyposazenie_sprzedaz () {
	FILE* plik = fopen ("Dane/wyposazenie_sprzedaz.csv", "w");

	if( plik == NULL ) {
		printf ("Failed to open file wyposazenie_sprzedaz.csv\n");
		return;
	}

	for( i = 1; i <= 10000; i++ ) {
		int ID_WyposazenieSprzedaz = i;
		int ID_Wyposazenie = random (1, 500);
		int ID_Sprzedaz = random (1, 10000);

		fprintf (plik, "%d,%d,%d\n", ID_WyposazenieSprzedaz, ID_Wyposazenie, ID_Sprzedaz);
	}
